...
def create_icon(source_image: NSImage, size: int = 1024) -> NSImage:
    ...
    # Draw source image scaled to fit the icon area
    ...
    bevel_offset = size * 0.004  # Offset amount
    stroke_width = size * 0.008  # Stroke width
    
    # Clip to squircle to keep bevel inside
    ctx.saveGraphicsState()
    squircle_path.addClip()
    
    # Draw highlight stroke (white, offset toward top-left, appears on top-left inner edge)
    ...
    # Draw shadow stroke (black, offset toward bottom-right, appears on bottom-right inner edge)
...
def main():
    ...
    if args.foreground:
        ...
        # For foreground, just copy the source image
