...
<p align="center">
  ...
  <br>
  Claude CodeとCursorとCodexのチャット履歴から作業日記を自動生成するmacOSアプリ
</p>
...
- **自動履歴収集** - Claude CodeとCursorとCodexのチャット履歴を自動で収集
...
3. AIプロバイダーを選択:
   ...
   - **OpenAI API** - OpenAI APIキーが必要
4. APIキーを入力
5. 「保存」をクリック

Settingsで選択できるOpenAIモデル:
- `gpt-5.2`
- `gpt-5-mini`（デフォルト）
- `gpt-5-nano`
...
デフォルトでは`~/Documents/CCDiary/`配下に`YYYY/MM/YYYY-MM-DD.md`形式で保存されます。設定から変更できます。
...
- `~/.codex/sessions/**/*.jsonl` - 現行のセッションイベントログ
- `~/.codex/sessions/**/*.json` - 旧セッション形式
